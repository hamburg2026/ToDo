...
export default function ConfirmDialog({ title, message, confirmLabel = 'Bestätigen', danger, onConfirm, onCancel }: Props) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#151f76]/35 p-4 animate-fade-in" onClick={onCancel}>
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-sm rounded-2xl glass p-6 shadow-glow animate-pop-in"
      >
        <div className="mb-3 flex items-center gap-2.5">
          <div className={`flex h-9 w-9 items-center justify-center rounded-full ${danger ? 'bg-rose-500/20 text-rose-600' : 'bg-violet-500/20 text-violet-300'}`}>
            <AlertTriangle size={17} />
          </div>
          <h2 className="text-base font-bold text-[#151f76]">{title}</h2>
        </div>
        <p className="mb-5 text-sm leading-relaxed text-[#151f76]/75">{message}</p>
