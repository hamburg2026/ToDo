  ...
  return (
    <>
      <div ref={menuRef} className="relative">
        <button
          onClick={() => setOpen((v) => !v)}
          className="flex items-center gap-1.5 rounded-full border border-[#151f76]/10 bg-[#151f76]/4 px-4 py-1.5 text-sm font-medium text-[#151f76]/85 transition-colors hover:bg-[#151f76]/6"
        >
        ...
        {open && (
          <div className="absolute right-0 top-[calc(100%+8px)] z-40 w-56 overflow-hidden rounded-xl glass p-1.5 shadow-glow animate-pop-in">
            ...
            <button
              ...
              className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm text-[#151f76]/90 hover:bg-[#151f76]/6"
            >
              <Upload size={15} className="text-[#0e8f7a]" />
              Backup laden
            </button>
            ...
            <button
              ...
              className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm text-rose-600 hover:bg-rose-500/10"
            >
              <RotateCcw size={15} />
              Zurücksetzen
            </button>
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-[70] flex items-center gap-2.5 rounded-xl glass px-4 py-3 text-sm text-[#151f76] shadow-glow animate-pop-in">
          {toast.ok ? <Check size={16} className="text-emerald-600" /> : <XIcon size={16} className="text-rose-500" />}
          {toast.text}
        </div>
      )}
    </>
